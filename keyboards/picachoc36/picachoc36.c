...
#ifdef PICACHOC36_RGBLIGHT_TIMEOUT
...
#endif // PICACHOC36_RGBLIGHT_TIMEOUT
...
void keyboard_post_init_kb(void) {
    setPinOutput(XIAO_RP2040_RGB_POWER_PIN);
...
void housekeeping_task_kb(void) {
    ...
    if (is_keyboard_master()) {
        if (timer_elapsed(check_rgblight_timer) > 1000) {
            ...
            if (rgblight_is_enabled() && counter > PICACHOC36_RGBLIGHT_TIMEOUT * 6) {
                counter = 0;
    ...
    if (timer_elapsed(check_layer_timer) > 100) {
        ...
        if (is_layer_active != current_is_layer_active) {
            ...
            if (is_layer_active) {
                writePinHigh(XIAO_RP2040_RGB_POWER_PIN);
            } else {
                writePinLow(XIAO_RP2040_RGB_POWER_PIN);
            }
        }
    }
#endif // defined(RGBLIGHT_ENABLE) && defined(RGBLIGHT_LAYERS)
